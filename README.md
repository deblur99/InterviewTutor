...
> **현재 버전:** 1.1 (갓·숙련·전문 단계 + 자유 연습·면접 일정·모의면접 기록)  
> **플랫폼:** macOS 26.0+  
> **개인정보:** Foundation Models, Speech, AVFoundation 등 Apple 온디바이스 API 중심 — 서버 전송 없음  
> **개인정보처리방침:** [InterviewTutor Privacy Policy](https://deblur99.github.io/InterviewTutor-privacy-policy/)
...
- 프로필 관리 화면에 **면접 D-Day** 표시 (일정 등록 시)
...
### 자유 연습
- **항목 선택:** 서류 / 인성 / 회사 / **이직 사유** / 자기소개 / 역질문 / 마지막으로 할 말 (복수 선택 가능)
- **문항 수:** 1~10개 (기본 2개)
- **옵션 변경 시 질문 재생성 디바운스** — Stepper·토글 조작 중 불필요한 FM 호출 방지
...
### 모의면접 기록
- 홈 **현재 프로필** 카드에 최근 연습 요약 (총 횟수, 최근 점수·등급, 최근 3회)
- **모의면접 기록** 시트 — 연습 추이 차트 + 역대 세션 목록 → 다시보기 이동
...
### About
- 시스템 메뉴 **About InterviewTutor** — GitHub, [개인정보처리방침](https://deblur99.github.io/InterviewTutor-privacy-policy/), LinkedIn, 이메일 링크
...
│   ├── Home/               # 홈, 일정·기록, 프로필 관리, 단계 선택
│   ├── Session/            # PreSession → Session → PostSession, 전문 설정
│   ├── Practice/           # 자유 연습 (항목 선택, 문항별·종합 피드백)
│   ├── Progress/           # 연습 추이 차트, 프로필 세션 통계
│   └── Replay/             # 세션 다시보기
└── SharedUI/               # 타이머 링, 배지, 점수 패널, About, 공통 버튼
```
...
`main` 브랜치 push·PR 시 GitHub Actions에서 단위·UI 테스트가 자동 실행됩니다.
...
| 시점 | 동작 |
|------|------|
| 홈 진입 / 프로필 전환·온보딩 완료 | **현재 프로필** 기준 갓·숙련·**전문** 풀 백그라운드 리필 |
...
> **자유 연습**은 세션 시작 시 항목·문항 수에 맞춰 즉시 생성하며, 구조화 단계용 풀과 분리됩니다.

## 개인정보 & 샌드박스

- App Sandbox 활성화 (Release entitlements 포함)
- 사용자가 선택한 파일만 읽기 (`files.user-selected.read-only`)
- 카메라·마이크는 세션 녹화·분석에만 사용
- **로컬 알림**은 면접 준비·D-Day 응원 메시지 예약에만 사용 (온디바이스, 서버 미전송)
- 면접 영상·프로필·세션 데이터는 로컬 `Application Support` / SwiftData에 저장
- 자세한 처리 방침: [개인정보처리방침](https://deblur99.github.io/InterviewTutor-privacy-policy/)
